...
export async function runMigrations(){
  ...
  if(!url)throw new Error('DATABASE_URL required for migrations');
  ...
  try{
    ...
    for(const file of files){
      ...
      if(exists.rowCount){console.log(`migration skip ${file}`);continue}
      const sql=await fs.readFile(path.join(dir,file),'utf8');
      console.log(`migration apply ${file}`);
      await client.query('begin');
      try{
        await client.query(sql);
        await client.query(`insert into nexoffice_migrations(name) values($1)`,[file]);
