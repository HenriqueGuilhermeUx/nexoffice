...
export async function registerBusinessOperationRoutes(app:FastifyInstance){
  ...
  app.post('/v1/business-operations/:id/communication-draft',async req=>{
    ...
    await auditLog(ctx,'business_operation.communication_draft_created','business_operation',id,null,{ledgerEntryId:operation.ledger_id,channel:'copy',externalEffect:false,secretPersisted:false});
    return {externalEffect:false,secretPersisted:false,channel:'copy',recipient:operation.contact_phone||null,contactName:operation.contact_name,message,smartBots:{paymentDataIncluded:false,note:'O texto completo com Pix permanece apenas na sessão autenticada do NexOffice.'}};
  });
  ...
  app.post('/v1/business-operations/:id/prepare-communication',async req=>{
    ...
    if(!operation.ledger_id)throw new ApiError(409,'collection_required','Prepare o recebível antes da comunicação.');
    if(operation.communication_action_id)throw new ApiError(409,'communication_already_prepared','Já existe um lembrete preparado para esta operação.');
    const recipient=digits(operation.contact_phone);if(recipient.length<10)throw new ApiError(409,'contact_whatsapp_required','Informe um WhatsApp válido no CRM antes de preparar o lembrete.');
    ...
    const message=[`Olá, ${operation.contact_name||'cliente'}!`,input.note||'',`Passando para lembrar da cobrança referente a ${operation.description}.`,`Valor: ${amount}.`,due?`Vencimento: ${due}.`:'',`Os dados de pagamento Pix são fornecidos diretamente pelo responsável do estabelecimento.`,operation.fiscal_external_ref?`Referência fiscal: ${operation.fiscal_external_ref}`:'','Se já realizou o pagamento, desconsidere esta mensagem.'].filter(Boolean).join('\n');
    const title=`Lembrete de cobrança · ${operation.contact_name||operation.title}`;
    const summary=`Lembrete de ${amount} preparado sem incluir chave Pix ou outro dado sensível de pagamento.`;
    const payload={channel:'whatsapp',recipient,contactName:operation.contact_name,message,ledgerEntryId:operation.ledger_id,businessOperationId:id};
    const created=await transaction(async client=>{
      ...
      const action=await client.query<any>(`insert into command_actions(workspace_id,agent_role,title,summary,priority,status,autonomy,approval_id,subject_type,subject_id,primary_action,secondary_actions,metadata) values($1,'collections',$2,$3,'high','open','approval_required',$4,'business_operation',$5,$6,$7,$8) returning *`,[ctx.workspaceId,title,summary,approval.rows[0].id,id,JSON.stringify({type:'message.send',payload}),JSON.stringify([]),JSON.stringify({source:'business-operation',provider:'smartbots',humanApprovalRequired:true,paymentDataIncluded:false,pixKeyPersisted:false})]);
      const updated=await client.query<any>(`update business_operations set communication_action_id=$3,status='communication_pending',metadata=metadata||$4::jsonb,updated_at=now() where id=$1 and workspace_id=$2 returning *`,[id,ctx.workspaceId,action.rows[0].id,JSON.stringify({communicationPreparedAt:new Date().toISOString(),communicationType:'collection_reminder',paymentDataIncluded:false,pixKeyPersisted:false})]);
      return {approval:approval.rows[0],action:action.rows[0],operation:updated.rows[0]};
    });
    await auditLog(ctx,'business_operation.communication_prepared','business_operation',id,operation,{actionId:created.action.id,approvalId:created.approval.id,provider:'smartbots',externalEffect:false,paymentDataIncluded:false,pixKeyPersisted:false});
    return {status:'pending_approval',operation:created.operation,action:created.action,approval:{id:created.approval.id,status:created.approval.status},governance:{humanApprovalRequired:true,externalEffect:false,paymentDataIncluded:false,pixKeyPersisted:false}};
  });
