...
export async function readCompanyLongitudinal(workspaceId:string){
  const [workspaceRows,health,risk,observations,signals,actions,briefs,metricQuality,eventCoverage]=await Promise.all([
    ...
    query<any>(`select count(*)::int events,min(occurred_at) first_event_at,max(occurred_at) last_event_at,count(distinct event_name)::int event_types from business_events where workspace_id=$1`,[workspaceId])
  ]);
