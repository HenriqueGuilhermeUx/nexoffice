...
export async function readCompanyLongitudinal(workspaceId:string){
  const [workspaceRows,health,risk,observations,signals,actions,briefs,metricQuality,eventCoverage,dataCoverageHistory]=await Promise.all([
    ...
    query<any>(`select run_date,coverage_pct,freshness_pct,automatic_pct,required_metrics,available_metrics,automatic_metrics,stale_metrics,history_days,status,gaps,sources,model_version,created_at,updated_at from intelligence_data_coverage_snapshots where workspace_id=$1 order by run_date desc limit 180`,[workspaceId])
  ]);
  const workspace=workspaceRows[0]||null;if(!workspace)return null;
  const firstDates=[health.at(-1)?.as_of,risk.at(-1)?.run_date,observations.at(-1)?.run_date,eventCoverage[0]?.first_event_at,dataCoverageHistory.at(-1)?.run_date].filter(Boolean).map((x:any)=>new Date(x).getTime()).filter(Number.isFinite);
  const latestDates=[health[0]?.as_of,risk[0]?.as_of,observations[0]?.updated_at,eventCoverage[0]?.last_event_at,dataCoverageHistory[0]?.updated_at].filter(Boolean).map((x:any)=>new Date(x).getTime()).filter(Number.isFinite);
  const coverage={
  ...
  const timeline=[
    ...
    ...dataCoverageHistory.slice(0,52).map((x:any)=>({kind:'coverage',at:x.updated_at||x.created_at||x.run_date,title:'Cobertura de dados',detail:`${Number(x.coverage_pct||0)}% de cobertura · ${Number(x.automatic_pct||0)}% automático · ${Number(x.history_days||0)} dias de histórico`,status:x.status,code:'data_coverage'}))
  ...
  return{
    ...
    coverage,
    dataCoverage:dataCoverageHistory[0]||null,
    dataCoverageHistory,
