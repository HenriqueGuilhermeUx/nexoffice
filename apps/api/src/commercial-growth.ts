...
export async function syncProjectLeadsToCrm(workspaceId:string,projectId:string){
  ...
  for(const lead of Array.isArray(leads)?leads:[]){const leadId=text(lead?.id);if(!leadId)continue;let [contact]=await query<any>(`select * from crm_contacts where workspace_id=$1 and custom_fields->>'modoLeadId'=$2 limit 1`,[workspaceId,leadId]);const source=`growth:${projectId}`,custom={modoLeadId:leadId,marketingProjectId:projectId,utm:asObject(lead?.utm),capturedAt:lead?.createdAt||null};if(contact){[contact]=await query<any>(`update crm_contacts set name=coalesce(nullif($3,''),name),email=coalesce(nullif($4,''),email),phone=coalesce(nullif($5,''),phone),source=$6,custom_fields=coalesce(custom_fields,'{}'::jsonb)||$7::jsonb,updated_at=now() where id=$2 and workspace_id=$1 returning *`,[workspaceId,contact.id,text(lead?.name),text(lead?.email),text(lead?.phone),source,JSON.stringify(custom)]);updated++}else{[contact]=await query<any>(`insert into crm_contacts(workspace_id,kind,name,email,phone,source,tags,custom_fields) values($1,'person',$2,$3,$4,$5,$6,$7::jsonb) returning *`,[workspaceId,text(lead?.name)||'Lead de campanha',text(lead?.email)||null,text(lead?.phone)||null,source,['growth','campaign'],JSON.stringify(custom)]);created++}const [existingDeal]=await query<any>(`select id from crm_deals where workspace_id=$1 and metadata->>'marketingLeadId'=$2 limit 1`,[workspaceId,leadId]);let dealId=existingDeal?.id||null;if(!dealId){const [deal]=await query<any>(`insert into crm_deals(workspace_id,contact_id,title,stage,value_minor,currency,source,next_action,metadata) values($1,$2,$3,'lead',0,'BRL',$4,'Qualificar lead da campanha',$5::jsonb) returning id`,[workspaceId,contact.id,`Lead de campanha · ${text(lead?.name)||'Novo contato'}`,source,JSON.stringify({marketingProjectId:projectId,marketingLeadId:leadId,utm:asObject(lead?.utm)})]);dealId=deal.id;dealsCreated++}synced.push({marketingLeadId:leadId,contactId:contact.id,dealId})}
