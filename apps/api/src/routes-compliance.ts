...
function bridgeConfigured(){return Boolean(safeBase(String(process.env.NR1CHECK_WEB_URL||''))&&bridgeSecret())}
...
function signedNr1checkUrl(workspaceId:string,businessName:string,sector:string){
  ...
  const encoded=Buffer.from(JSON.stringify(payload)).toString('base64url'),signature=createHmac('sha256',secret).update(encoded).digest('base64url');
  const url=new URL('/nexoffice',base);url.searchParams.set('handoff',`${encoded}.${signature}`);return{url:url.toString(),expiresAt:new Date(payload.exp*1000).toISOString()};
}

function acceptedComplianceDeepLink(value:unknown){
  if(typeof value!=='string'||!value.trim())return null;
  try{const url=new URL(value),expected=safeBase(String(process.env.NR1CHECK_WEB_URL||''));if(!['https:','http:'].includes(url.protocol))return null;if(expected&&url.origin!==new URL(expected).origin)return null;return url.toString()}catch{return null}
}

const Summary=z.object({
  workspaceRef:z.string().uuid(),sourceProduct:z.enum(['nr1check','mindcompliance']),diagnosticStatus:z.enum(['not_started','in_progress','completed','not_applicable']).optional(),programStatus:z.enum(['not_started','active','attention','up_to_date']).optional(),openActions:z.number().int().min(0).max(100000).default(0),overdueActions:z.number().int().min(0).max(100000).default(0),nextDueAt:z.string().datetime().nullable().optional(),completionPct:z.number().min(0).max(100).nullable().optional(),categories:z.array(z.string().trim().min(1).max(80)).max(20).default([]),deepLink:z.string().url().max(2000).nullable().optional(),observedAt:z.string().datetime().optional(),
}).strict();
type SummaryInput=z.infer<typeof Summary>;

function sanitizeSummary(payload:any,workspaceId:string):SummaryInput|null{
  ...
  if(!parsed.success||parsed.data.workspaceRef!==workspaceId)return null;
  return{...parsed.data,deepLink:acceptedComplianceDeepLink(parsed.data.deepLink)};
}

async function persistSummary(workspaceId:string,input:SummaryInput){
  const workspace=(await query<any>(`select id,settings from workspaces where id=$1`,[workspaceId]))[0];if(!workspace)return false;
  const current=workspace.settings&&typeof workspace.settings==='object'?workspace.settings:{},previousCompliance=current.compliance&&typeof current.compliance==='object'?current.compliance:{};
  ...
  await query(`update workspaces set settings=coalesce(settings,'{}'::jsonb)||jsonb_build_object('compliance',$2::jsonb),updated_at=now() where id=$1`,[workspaceId,JSON.stringify({...previousCompliance,summary})]);return true;
}

async function pullComplianceSummary(workspaceId:string){
  const apiBase=safeBase(String(process.env.NR1CHECK_API_URL||'')),secret=bridgeSecret();if(!apiBase||!secret)return null;
  try{
    const response=await fetch(`${apiBase}/api/internal/nexoffice/compliance-summary?workspaceRef=${encodeURIComponent(workspaceId)}`,{headers:{accept:'application/json','x-nexoffice-compliance-key':secret},signal:AbortSignal.timeout(5000)});
    if(response.status===404)return null;if(!response.ok)return null;const payload=await response.json() as any;const summary=sanitizeSummary(payload,workspaceId);if(!summary)return null;await persistSummary(workspaceId,summary);return summary;
  }catch{return null}
}
...
async function overview(workspaceId:string){
  ...
  const remote=await pullComplianceSummary(workspaceId),freshWorkspace=remote?(await query<any>(`select settings from workspaces where id=$1`,[workspaceId]))[0]:workspace;
  const settings=freshWorkspace?.settings&&typeof freshWorkspace.settings==='object'?freshWorkspace.settings:{},compliance=settings.compliance&&typeof settings.compliance==='object'?settings.compliance:{};
  const triage=Triage.parse(compliance.triage||{}),relevance=relevanceFor(triage),sector=String(profile?.sector||workspace.vertical||'general'),signed=signedNr1checkUrl(workspaceId,workspace.name,sector),summary=remote||((compliance.summary&&typeof compliance.summary==='object')?compliance.summary:null);
  return{available:true,workspace:{id:workspace.id,name:workspace.name,sector},triage,relevance,nr1check:{configured:Boolean(signed),launchUrl:signed?.url||null,signedHandoff:Boolean(signed),expiresAt:signed?.expiresAt||null,liveSummary:liveSummaryConfigured()},mindcompliance:{connected:Boolean(summary),summary},privacy:{mode:'aggregate_only',sourceOwnsCanonicalData:true,forbidden:['employee_personal_data','cpf','health_data','psychosocial_answers','complaint_content','raw_documents','confidential_evidence']},legalGuidance:{automaticLegalConclusion:false,note:'A triagem indica relevância e próximo passo. Ela não substitui avaliação normativa ou profissional.'}};
}
...
export async function registerComplianceRoutes(app:FastifyInstance){
  ...
  app.post('/v1/compliance/nr1check/handoff',async req=>{const ctx=await workspaceContext(req,'integrations.read'),data=await overview(ctx.workspaceId),signed=signedNr1checkUrl(ctx.workspaceId,data.workspace.name,data.workspace.sector);if(!signed)throw new ApiError(503,'nr1check_not_configured','NR1Check ainda não está configurado neste ambiente.');await auditLog(ctx,'compliance.nr1check.handoff.created','workspace',ctx.workspaceId,null,{source:'nexoffice',mode:'signed',sharedBusinessFields:['workspaceRef','businessName','sector'],identityInBrowserUrl:false,sensitiveBusinessDataShared:false,expiresAt:signed.expiresAt,externalEffect:false});return{url:signed.url,mode:'signed',expiresAt:signed.expiresAt,sharedBusinessFields:['workspaceRef','businessName','sector'],identityInBrowserUrl:false,sensitiveBusinessDataShared:false}});
