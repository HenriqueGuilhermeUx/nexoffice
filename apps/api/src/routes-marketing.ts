...
export async function registerMarketingRoutes(app:FastifyInstance){
  app.get('/v1/marketing/status',async req=>{
    ...
    if(!modoMarketingConfigured())return {configured:false,provider:'modo',contract:'nexoffice-marketing-v1',externalCampaignActivation:false,projects:[],campaigns:[],connections:[]};
    try{
      const [health,projects,campaigns,connections]=await Promise.all([
        modoMarketingRequest(ctx.workspaceId,'health'),modoMarketingRequest(ctx.workspaceId,'demand/projects'),modoMarketingRequest(ctx.workspaceId,'campaigns'),modoMarketingRequest(ctx.workspaceId,'media/connections')
      ]);
      return {configured:true,provider:'modo',health,projects,campaigns,connections,externalCampaignActivation:false};
  ...
  app.post('/v1/marketing/media/:provider/prepare',async req=>{const ctx=await workspaceContext(req,'integrations.manage'),provider=z.enum(['google_ads','meta_ads']).parse((req.params as any).provider);try{const prepared=await modoMarketingRequest(ctx.workspaceId,`media/connections/${provider}/prepare`,'POST',{});await auditLog(ctx,'marketing.media.prepare','workspace',ctx.workspaceId,null,{provider,externalEffect:false,officialOAuthRequired:true});return prepared}catch(e){return fail(e)}});
  ...
  app.post('/v1/marketing/campaigns/:id/ready',async req=>{
    const ctx=await workspaceContext(req,'integrations.manage'),id=z.string().uuid().parse((req.params as any).id),input=z.object({approved:z.literal(true)}).parse(req.body);
    try{const result=await modoMarketingRequest(ctx.workspaceId,`campaigns/${id}/ready`,'POST',input);await auditLog(ctx,'marketing.campaign.ready','marketing_campaign',id,null,{clientApproved:true,externalEffect:false});return result}catch(e){return fail(e)}
  });
}
