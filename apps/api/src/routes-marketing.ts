...
export async function registerMarketingRoutes(app:FastifyInstance){
  ...
  app.post('/v1/marketing/content/drafts/:id/approve',async req=>{const ctx=await workspaceContext(req,'integrations.manage'),id=z.string().uuid().parse((req.params as any).id);try{const result=await modoMarketingRequest<any>(ctx.workspaceId,`content/drafts/${id}/approve`,'POST',{});await auditLog(ctx,'marketing.content.draft.approved','content_draft',id,null,{explicitApproval:true,externalEffect:false,externalPublication:false});return result}catch(e){return fail(e)}});
