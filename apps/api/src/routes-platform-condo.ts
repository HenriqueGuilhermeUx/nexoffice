...
async function condoWorkspace(externalWorkspaceRef:string){
  const workspace=(await query<any>(`select w.id,w.name,w.vertical from workspace_origins o join workspaces w on w.id=o.workspace_id where o.source_product='sindcopilot' and o.external_workspace_ref=$1 limit 1`,[externalWorkspaceRef]))[0];
  ...
  if(workspace.vertical!=='condo')throw new ApiError(409,'condo_workspace_required','Sinais condominiais só podem entrar em workspace do vertical condo.');
  return workspace;
}

export async function registerPlatformCondoRoutes(app:FastifyInstance){
  app.post('/v1/platform/condo-signals',async req=>{
    authorize(req);
    const input=CondoOperationalSignal.parse(req.body);
    const workspace=await condoWorkspace(input.externalWorkspaceRef);
    const rows=await query<any>(`
      ...
      values($1,'sindcopilot',$2,$3,$4,$5,$6,$7)
  ...
  app.get('/v1/platform/condo-signals',async req=>{
    authorize(req);
    const input=z.object({sourceProduct:z.literal('sindcopilot').default('sindcopilot'),externalWorkspaceRef:z.string().trim().min(1).max(220),limit:z.coerce.number().int().min(1).max(100).default(30)}).strict().parse(req.query);
    const workspace=await condoWorkspace(input.externalWorkspaceRef);
    const rows=await query<any>(`select id,source_product,signal_type,period_start,period_end,metrics,dimensions,correlation_id,created_at from workspace_operational_signals where workspace_id=$1 and source_product='sindcopilot' order by created_at desc limit $2`,[workspace.id,input.limit]);
    return {workspace:{id:workspace.id,name:workspace.name,vertical:workspace.vertical},privacy:'aggregate_only',allowedSignalTypes:CONDO_SIGNAL_TYPES,signals:rows};
  });
}
