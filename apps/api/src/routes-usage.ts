...
export async function registerUsageRoutes(app:FastifyInstance){
  app.get('/v1/usage/summary',async req=>{
    ...
    const [totals,providers,capabilities,daily,workspace]=await Promise.all([
      ...
      query<any>(`select date_trunc('day',occurred_at)::date as usage_day,count(*)::int events,coalesce(sum(units),0)::numeric units,coalesce(sum(cost_minor_estimate) filter(where cost_minor_estimate is not null),0)::bigint known_cost_minor,count(*) filter(where cost_minor_estimate is null)::int unknown_cost_events from usage_events where workspace_id=$1 and occurred_at>=now()-($2::text||' days')::interval group by 1 order by 1`,[ctx.workspaceId,String(days)]),
    ...
    const events=Number(total.events||0),unknownCostEvents=Number(total.unknown_cost_events||0),knownCostEvents=Math.max(0,events-unknownCostEvents);
    const knownCost=Number(total.known_cost_minor||0),observedActiveDays=daily.length,projectionDivisor=Math.max(1,observedActiveDays),avgKnownDaily=Math.round(knownCost/projectionDivisor);
    const projection30=observedActiveDays?avgKnownDaily*30:0;
    const catalog=meteringCatalog();
    const configuredTopics=catalog.filter(item=>item.costMinorEstimate!==null).length;
    const ratio=(value:number,totalValue:number)=>totalValue>0?Number((value/totalValue).toFixed(4)):null;
    return {
      ...
      totals:{events,units:Number(total.units||0),knownCostMinor:knownCost,unknownCostEvents,firstEventAt:total.first_event_at||null,lastEventAt:total.last_event_at||null},
      measurement:{observedActiveDays,requestedWindowDays:days,activeDayRatio:ratio(observedActiveDays,days),knownCostEvents,unknownCostEvents,costedEventRatio:ratio(knownCostEvents,events),configuredCostTopics:configuredTopics,totalMeteredTopics:catalog.length,configuredTopicRatio:ratio(configuredTopics,catalog.length),observedProviders:providers.length,observedCapabilityOperations:capabilities.length},
      economics:{currency:'BRL',activeUsageDays:observedActiveDays,averageKnownCostPerActiveDayMinor:avgKnownDaily,projected30DayKnownCostMinor:projection30,projectionBasis:'known_cost_only',costCoverage:{configuredTopics,totalTopics:catalog.length,complete:configuredTopics===catalog.length},warning:unknownCostEvents>0?'A projeção não inclui eventos cujo custo unitário ainda não foi configurado.':null},
      providers:providers.map(normalizeRow),capabilities:capabilities.map(normalizeRow),daily:daily.map(row=>({...normalizeRow(row),day:String(row.usage_day)})),meteringCatalog:catalog
    };
  });
