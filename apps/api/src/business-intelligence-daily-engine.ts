...
import {buildSevenDayPlan,syncActivationJourneys} from './business-founder-cockpit.js';
...
export type DailyIntelligenceEngineSummary={
  ...
  activationJourneysActivated:number;weeklyPlans:number;
...
const emptySummary=(claimed:boolean):DailyIntelligenceEngineSummary=>({claimed,workspaces:0,dataMetricsWritten:0,dataCoverageStrong:0,dataCoverageUsable:0,dataCoverageForming:0,financialSnapshots:0,businessSnapshots:0,temporalRiskSnapshots:0,temporalSignals:0,sectorObservations:0,sectorSignals:0,evaluatedActions:0,improved:0,stable:0,worsened:0,insufficient:0,temporalActionsEvaluated:0,activationJourneysActivated:0,weeklyPlans:0,suggestionsCreated:0,benchmarkCohorts:0,benchmarkMetricRows:0,benchmarkEligibleWorkspaces:0,errors:0,errorSamples:[]});
...
export async function runDailyIntelligenceEngine(force=false):Promise<DailyIntelligenceEngineSummary>{
  ...
  try{
    ...
    const growthLoopSchema=Boolean((await query<any>(`select to_regclass('public.intelligence_activation_journeys') is not null and to_regclass('public.intelligence_7day_plans') is not null ready`))[0]?.ready);
    ...
    for(const workspace of workspaces){
      ...
      try{
        ...
        if(growthLoopSchema){const activation=await syncActivationJourneys(workspaceId);summary.activationJourneysActivated+=Number(activation.activated||0);await buildSevenDayPlan(workspaceId);summary.weeklyPlans++}
