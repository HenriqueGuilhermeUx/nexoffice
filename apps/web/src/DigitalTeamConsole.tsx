...
export default function DigitalTeamConsole(){
  ...
  const [text,setText]=useState('');const [brief,setBrief]=useState<Brief|null>(null);const [busy,setBusy]=useState(false);const [error,setError]=useState('');
  ...
  useEffect(()=>{if(!authenticated||!active){setBrief(null);return}void loadBrief()},[sessionKey,active]);
  async function loadBrief(){try{setBrief(await api<Brief>('/v1/assistant/brief'))}catch{}}
  ...
  return <aside className="digitalTeamConsole" aria-label="Equipe Digital NexOffice">
    ...
    <div className="digitalTeamFoot"><span>Contexto compartilhado</span><span>Memória empresarial</span><span>Approval-first</span></div>
  </aside>;
}
