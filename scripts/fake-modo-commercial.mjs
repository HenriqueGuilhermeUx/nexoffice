...
const server=http.createServer(async(req,res)=>{const u=new URL(req.url||'/',`http://127.0.0.1:${port}`),p=u.pathname.replace('/api/v1/internal/nexoffice/marketing/v1/','');if(req.headers['x-nexoffice-key']!=='test-key')return send(res,401,{error:'unauthorized'});
 ...
 if(p==='content/drafts'&&req.method==='POST'){
   const b=await read(req),id=randomUUID(),unsafe=String(b?.brief||'').includes('[TEST_UNSAFE_CLAIM]');
   const output=unsafe
    ?{hook:'Garantimos 50% mais resultados',title:'A melhor do mercado por apenas R$ 99',caption:'Promoção por tempo limitado com clientes satisfeitos e certificação garantida.',cta:'Quero o desconto',hashtags:[],visualDirection:'Número 1 do mercado',slides:[],script:[],storyFrames:[],adaptationNotes:[]}
    :{hook:'Organize seu próximo passo comercial',title:'Cresça com uma operação mais organizada',caption:'Conheça a oferta registrada e fale com a equipe para entender se ela faz sentido para seu negócio.',cta:'Quero saber mais',hashtags:[],visualDirection:'Visual profissional sem alegações adicionais',slides:[],script:[],storyFrames:[],adaptationNotes:[]};
   const request={id,...b,status:'ready',creditsCharged:0,revisionCount:0,maxRevisions:2,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),output};drafts.set(id,request);return send(res,201,{request,governance:{billingMode:'nexoffice_entitlement',modoCreditsCharged:0,publishing:false,externalPublication:false}})}
