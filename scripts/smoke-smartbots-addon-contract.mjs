...
expect(revenueRoutes.includes('playbooks:playbooksFor(segment)'),'Revenue overview must expose segment-aware playbooks');
expect(revenueRoutes.includes("type RevenueSegment='health_services'|'professional_services'|'commerce'|'general'"),'Revenue playbook segment contract missing');
expect(revenueRoutes.includes("app.post('/v1/revenue/playbooks/prepare'"),'Revenue playbook preparation endpoint missing');
expect(revenueRoutes.includes("playbookKind:input.kind"),'Prepared actions must retain playbook identity');
expect(revenueRoutes.includes('revenueSegment:segment'),'Prepared actions must retain segment identity');
...
expect(revenueUi.includes("post<any>('/v1/revenue/playbooks/prepare'"),'Revenue UI must prepare playbooks through governed API');
...
expect(revenueUi.includes('PLAYBOOKS DO NEGÓCIO'),'Revenue UI must surface segment-aware playbooks');
expect(revenueUi.includes('Aplicar playbook'),'Revenue UI must make stalled-pipeline playbook actionable');
expect(revenueUi.includes('Nenhuma mensagem foi enviada'),'Playbook UI must disclose preparation-only behavior');
