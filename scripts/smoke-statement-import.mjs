...
const contact=await call('/v1/crm/contacts',{method:'POST',body:{kind:'person',name:'Cliente Alpha',tags:['statement-smoke'],customFields:{}}});
await call('/v1/ledger',{method:'POST',body:{contactId:contact.id,accountId:account.id,direction:'income',category:'vendas',description:'Cliente Alpha',amountMinor:125050,currency:'BRL',status:'open',dueAt:'2026-09-15T12:00:00.000Z'}});
...
const ofx='<OFX><BANKMSGSRSV1><STMTTRNRS><STMTRS><BANKTRANLIST><STMTTRN><TRNTYPE>CREDIT<DTPOSTED>20260917120000<TRNAMT>89.90<FITID>OFX-SMOKE-001<NAME>Cliente OFX<MEMO>Pagamento teste</STMTTRN></BANKTRANLIST></STMTRS></STMTTRNRS></BANKMSGSRSV1></OFX>';
const ofxPreview=await call('/v1/reconciliation/import-file/preview',{method:'POST',body:{filename:'extrato.ofx',content:ofx}});assert(ofxPreview.format==='ofx','ofx detected');assert(ofxPreview.count===1,'one OFX transaction previewed');assert(ofxPreview.sample[0].amountMinor===8990,'OFX amount parsed in minor units');
...
console.log(JSON.stringify({ok:true,workspace,formats:[preview.format,ofxPreview.format],imported:imported.inserted,duplicates:duplicate.duplicates,matched:rec.matched,rawFileStored:imported.rawFileStored},null,2));
