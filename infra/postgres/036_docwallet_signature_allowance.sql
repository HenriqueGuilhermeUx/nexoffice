...
create table if not exists docwallet_signature_usage (
  ...
  status text not null default 'processing' check(status in ('processing','pending','completed','cancelled')),
...
comment on table docwallet_signature_usage is 'Monthly usage/reservation ledger for documents sent to DocWallet signature. One signature request/document counts as one included usage regardless of number of parties.';
comment on column docwallet_signature_usage.idempotency_key is 'Stable bridge key reserved before the external DocWallet call, allowing safe recovery without duplicate signature requests.';
comment on column docwallet_signature_usage.status is 'processing reserves allowance before the remote call; retry reuses the same idempotency key. pending/completed/cancelled are established DocWallet outcomes.';
